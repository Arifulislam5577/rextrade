...
const HEADER_OFFSET = '-80px 0px 0px 0px'
...
export function SiteHeader() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        setIsPastHero(entry ? !entry.isIntersecting : false)
      },
      { rootMargin: HEADER_OFFSET, threshold: 0 },
    )
  ...
  const isSolid = isPastHero || isMenuOpen
  ...
  return (
    <header
      className={cn(
        ...
        isSolid
          ? 'border-hairline bg-white/90 shadow-sm shadow-sky-100 backdrop-blur'
          : 'border-transparent bg-transparent',
      )}
    >
      <div className="main-container relative flex h-16 items-center justify-between gap-6 lg:h-20">
        <Link href="/" className="flex items-center gap-3" onClick={handleMenuClose}>
          ...
          <span className="flex flex-col leading-none">
            <span className="font-display text-ink text-lg font-extrabold tracking-tight">
              ReXTrade
            </span>
            <span className="eyebrow text-slate-body mt-1">International</span>
          </span>
        </Link>
        ...
        <div
          ...
          hidden={!isMenuOpen}
          className="border-hairline absolute inset-x-0 top-full border-b bg-white shadow-lg shadow-sky-100 lg:hidden"
        >
          <nav aria-label="Mobile" className="main-container py-6">
